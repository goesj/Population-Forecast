...
model {
  ...
  target += normal_lpdf(m|0, 200); 
  
  // priors of variances
  target += normal_lpdf(s|0,100);
  
  target += normal_lpdf(sigma|sig_mean, sig_sd);

  target += normal_lpdf(sig_mean| 0 , 50);
  target += normal_lpdf(sig_sd| 0, 20);
  // 
  //target += normal_lpdf(sigma|50, 500); 
  ...
  for(r in 1:R){ // for all regions
    target += normal_skewed_lpdf(y[r,1]|mu[r]*(1-a_r[r]), sigma[r], gamma);
    for(t in 2:T){
...
generated quantities{
  ...
  int pos = 1;
   ...
   // In-Sample Measures
   for(r in 1:R){
     loglike[pos] = normal_skewed_lpdf(y[r, 1] | mu[r]*(1-a_r[r]), sigma[r], gamma);
